...
# Dataaa <-read.csv('../Data/Data-V3.csv',sep=',',header=TRUE)
# Dataaa <- Dataaa[,-1]
# saveRDS(Dataaa, "city.rds")
# Datahaha <- readRDS(file="city.rds") 



# ---------------------------------------------------------------------------------
# 洗数据
deletee2 <- which(DataOriginal1$Date)
df[which(DataOriginal1$DateD == 2016-01-02),]
DataOriginal1 <- DataOriginal1[-deletee1,]

df2 <- Data[2185:37271,]  # 选出我要的日期
df <- df[,c(1,3,5)]# 选出我不要的列
# ---------------------------------------------------------------------------------
...
Data <- readRDS(file="../data/Data_RTE.RData") #
...
# write.csv(Data,"/Users/utility/Desktop/DisserDraft/aaaaaaatest.csv")

# Datetime for training and testing dataset seperation
date_fin <- as.POSIXct(strptime("2013-10-15 11:30:00", "%Y-%m-%d %H:%M:%S"), tz="UTC")
date_fin2 <- as.POSIXct(strptime("2014-02-15 11:30:00", "%Y-%m-%d %H:%M:%S"), tz="UTC")
