...
result_innormal <- data.frame(row.names = c("MAPE" ,
                                            "Chi2.CR" ,
                                            "Chi2.PointCR",
                                            "Chi2.AvL",
...
# 手动    开始      打开原始数据
readRDS(file="../data/Data_All.RData")
# 固定
...
for(j in 11:15){
  ...
  result_innormal <- rbind(result_innormal, innormal)
...
saveRDS(result_groupbymonth, "result_groupbymonth_1-10.RData") # 存数据
saveRDS(result_innormal, "result_innormal_1-10.RData") # 存数据
#02:26
